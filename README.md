...
本项目先复现原生 LeWM 的视觉世界模型规划流程，再研究一个更具挑战的问题：**能否让仅用短期动作窗口训练的 LeWM，在不重新训练世界模型的条件下，辅助机械臂完成完整的长期夹取和转移任务？**

研究过程分为三个阶段：复现并发现问题、围绕根因设计新架构、用改造后的闭环系统验证抓握与转移能力。
